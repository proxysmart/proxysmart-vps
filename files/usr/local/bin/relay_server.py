...
async def recv_message(websocket ) -> tuple[int, bytes] | None:
    """reads (channel_id, data) from websocket - for VPS<>backconnect-client """
    try:
...
async def send_message(websocket, cid: int, data: bytes):
    """sends message 2 backconnect-client"""
    #print('=> PC', cid, data[:100])
...
async def handle_backconnect_client ( websocket ):
    ...
    try:
        ...
        # loop getting messages from PC, that go to external WEB clients
        while True:
...
async def close_channel(cid: int, send_close_to_pc: bool = True):
    ...
    try:
        ...
        pass
    # notify backconnect-client close channe (empty msg)
    if send_close_to_pc:
        #print(33333)
        ...
        if client_name:
            ...
            if bc_info :
                ...
                await send_message( websocket, cid, b'') 
...
#            client_name=m.group(1)
    ...
    try:
        # run forward_from_external in bg
        fwd_task = asyncio.create_task(forward_from_external())
        # wait till channel is closed (close_channel is troggered on error / closing ext. client)
        await fwd_task
    except:
        pass
    finally:
